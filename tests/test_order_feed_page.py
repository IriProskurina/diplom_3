...
class TestOrderFeedPage:
    ...
    def test_completed_orders_increase_when_user_make_order(self, driver, user, counter, order_feed_page):
        ...
        # Ожидаем, пока счетчик изменится
        WebDriverWait(driver, 30).until(
            lambda x: order_feed_page.get_completed_orders_count(counter) > orders_count_before,
            message=f"Счетчик не обновился после отправки заказа. Было: {orders_count_before}, сейчас: {order_feed_page.get_completed_orders_count(counter)}"
        )
